...
glm::dvec4 RT::Texture::Image::GetColor(const glm::dvec2& uvCoords)
{
	glm::dvec4 outputColor(1.0);
	...
	else
	{
		...
		if ((x >= 0) && (x < m_xSize) && (y >= 0) && (y < m_ySize))
		{
			...
			outputColor[0] = interpR / 255.0;
			outputColor[1] = interpG / 255.0;
			outputColor[2] = interpB / 255.0;
			outputColor[3] = interpA / 255.0;
		}
	}
...
double RT::Texture::Image::LinearInterp(const double& x0, const double& y0, const double& x1, const double& y1, const double x)
{
	double output;
...
double RT::Texture::Image::BilinearInterp(const double& x0, const double& y0, const double& v0, const double& x1, const double& y1, const double& v1, const double& x2, const double& y2, const double& v2, const double& x3, const double& y3, const double& v3, const double& x, const double& y)
{
...
void RT::Texture::Image::GetPixelValue(int x, int y, double& red, double& green, double& blue, double& alpha)
{
	if ((x >= 0) && (x < m_xSize) && (y >= 0) && (y < m_ySize))
	{
